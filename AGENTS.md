...
**当前实现状态**: 已具备基础错题分析能力，包括根据 `weak_points` 关键词和四维评分识别错因，输出证据、补救建议、下一步动作和 Markdown。相似错误检索、补救题推荐后续会和 LinkerAgent 继续整合。
...
tools:
  ...
  - find_similar_mistakes: 查找相似错误（规划）
  - generate_comparison_table: 生成对比表（当前以补救动作模板形式支持）
  - recommend_remedial_questions: 推荐补救题目（规划）
```
...
analysis = analyzer.analyze_wrong_answer(
    {
