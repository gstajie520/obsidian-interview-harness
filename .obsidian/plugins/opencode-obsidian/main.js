...
var WindowsProcess = class _WindowsProcess {
  static {
    // Static state to track the current process for cleanup
    this.currentProcess = null;
  }
  static {
    this.cleanupHandlerRegistered = false;
  }
  ...
  static registerCleanupHandler() {
    ...
    if (typeof window !== "undefined" && !process.env.CI) {
      window.addEventListener("beforeunload", () => {
        if (_WindowsProcess.currentProcess?.pid) {
...
var WorkspaceContext = class {
  ...
  gatherContext(maxNotes, maxSelectionLength) {
    ...
    for (const leaf of leaves) {
      ...
      const path = view2.file?.path;
      if (path) {
        paths.add(path);
      }
    }
    const openNotePaths = Array.from(paths).slice(0, Math.max(0, maxNotes));
    const view = this.app.workspace.getActiveViewOfType(import_obsidian4.MarkdownView) ?? this.lastMarkdownView;
